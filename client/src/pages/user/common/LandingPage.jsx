...
const LandingPage = () => {
    ...
    const fetchTutors = useCallback(async () => {
        ...
        try {
            ...
            const userResponse = await api.get("users/dashboard/admin-dashboard/")
            console.log('tutors response:', response, userResponse);
            setTutors(response.data.tutors || [1, 2, 3, 4]);
            setCourses(response.data.courses || []);
            setCourseDetails(response.data.course_details || []); 
            setUsersDetails(userResponse.data || []);
        } catch (err) {
            console.log("err:", err);
        } finally {
            setLoading(false);
        ...
        <div className="min-h-screen flex flex-col">
            <main className="flex-grow">
                    ...
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 relative">
                        ...
                        <div className="space-y-6">
                            ...
                            <p className="text-sm text-muted-foreground">
                                Joined by <span className="text-foreground font-semibold">{usersDetails?.active_users?.toLocaleString()}+</span> learners
                            </p>
                            </div>
                        </div>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="p-4">
                        <div className="text-3xl md:text-4xl font-bold text-accent">{courseDetails?.total_courses?.toLocaleString()}+</div>
                        <div className="text-sm text-muted-foreground mt-1">Courses Available</div>
                    </div>
                    <div className="p-4">
                        <div className="text-3xl md:text-4xl font-bold text-accent">{courseDetails?.total_students?.toLocaleString()}+</div>
                        <div className="text-sm text-muted-foreground mt-1">Active Learners</div>
                    </div>
                    <div className="p-4">
                        <div className="text-3xl md:text-4xl font-bold text-accent">{courseDetails?.total_instructors?.toLocaleString()}+</div>
                        <div className="text-sm text-muted-foreground mt-1">Expert Instructors</div>
                    </div>
                    <div className="p-4">
                        <div className="text-3xl md:text-4xl font-bold text-accent">{courseDetails.completed_courses && courseDetails.total_purchases ? ((courseDetails.completed_courses / courseDetails.total_purchases) * 100).toLocaleString('en-IN', { maximumFractionDigits: 1 }) : 0}%</div>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <Link to={!role ? "/register" : "/student/courses"} className="btn-outline mt-4 md:mt-0 flex items-center gap-2">
                        {`${!role ? "Register to view" : "View"} All Courses`} <ArrowRight size={16} />
                    </Link>
                    </div>
                    ...
                    {courses.map((course) => (
                        <div key={course.id} className="course-card border border-border">
                            ...
                            <div className="p-4 flex flex-col justify-between h-52">
                                ...
                                <p className="truncate mb-2 font-extralight text-muted-foreground">
                                    {course.description}
                                </p>                               
                                <div className="flex justify-between items-center mb-4">
                                    <div className="flex items-center">
                                        ...
                                        {Array(5).fill(0).map((_, i) => (
                                            ...
                                            height="16" 
                                            viewBox="0 0 24 24" 
                                            fill={i < Math.floor(course.average_rating) ? "currentColor" : "none"} 
                                            stroke="currentColor"
                ...
                {/* CTA Section */}
                {!role && <section className="py-16 relative overflow-hidden">
                    {/* Background gradient effect */}
                    ...
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            ...
                            <Link to="/login" className="btn-outline text-center px-8 py-3">
                                Login Now
                            </Link>
                        </div>
