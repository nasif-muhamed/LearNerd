...
const StudentCourseDetails = () => {
  const courseData = {
    ...
    subtitle: "Get 100% job-ready in HTML, CSS, JS, React, Node, MongoDB, SQL, Git, + more!",
    skills: ["Frontend", "Web Dev", "Fullstack"],
    creator: "Harsh Chaudhary",
    uploadDate: "12/21/2024",
    rating: 4.7,
    students: 256032,
    hours: "64h 24m",
    sections: 8,
    ...
    whatYouLearn: [
      ...
      "Build real projects for your company or as portfolio projects",
      "Master HTML, CSS, JavaScript concepts",
      "Work with REST API and many advance topics"
    ],
    requirements: [
    ...
    features: [
      { text: "773 hours on-demand video", premium: false },
      { text: "45 coding exercises", premium: false },
      { text: "5 end-to-end exercises", premium: true },
      { text: "22 downloadable resources", premium: false },
      { text: "1 month email assistance", premium: true },
    ],
    instructor: {
      name: "Harsh Chaudhary",
      title: "A teacher who loves to teach about Technology",
      avatar: "https://images.unsplash.com/photo-1568602471122-7832951cc4c5?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80",
      rating: "4,640",
      students: 94674,
      courses: 1,
      bio: "A teacher who loves to teach about Technology with over 10 years of experience in web development and teaching thousands of students worldwide."
    },
    reviews: [
      {
        name: "Yousef",
        ...
        rating: 5,
        time: "3 months ago",
        comment: "Thanks Harsh Chaudhary for this - it was pretty cool for me to learn frontend development which I was looking into since long. You have beautifully covered everything. Thank you for your helpful Boot Course for full-stack."
      },
      {
        name: "Harsh K.",
        ...
        rating: 5,
        time: "3 months ago",
        comment: "Awesome, I loved his teaching style and the videos are very impactful."
      }
    ],
    similarCourses: [
      {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-6">
              <div className="bg-navy-light rounded-lg overflow-hidden mb-6">
                ...
                <div className='p-4'>
                  <div className={`flex ${course?.subscription ? 'flex-col' : 'flex-row gap-10'}`}>
                    <div className="flex justify-between items-center mb-4">
                      <button className="text-red-600 hover:text-red-500 transition-colors duration-300" title='wishlist'>
                      ...
                      {course?.subscription &&  (<div className="text-2xl font-bold">₹{Number(course?.subscription_amount).toLocaleString('en-US')}</div>)}
                    </div>
                                      
                    {course?.subscription && (<button className="btn-primary w-full mb-2">
                      Subscribe
                    </button>)}
                    
                    <button className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-md transition-all duration-300 flex items-center justify-center mb-4">
                  ...
                  <div className="text-sm">
                    <div className="font-medium mb-1">This course includes:</div>
                    <ul className="space-y-2">
                      <li className="flex items-center">
                        <svg className="w-4 h-4 mr-2 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd"></path>
                        </svg>
                        <span>{course?.analytics?.video_count} on-demand videos</span>
                      </li>
                      <li className="flex items-center">
                        <svg className="w-4 h-4 mr-2 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                          <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd"></path>
                        </svg>
                        <span>{course?.analytics?.assessment_count} assessments</span>
                      </li>
                      <li className="flex items-center">
                        <svg className="w-4 h-4 mr-2 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="lg:col-span-2">
            <CourseHeader 
              ...
              creator={course?.instructor_details?.first_name + " " + course?.instructor_details?.last_name}
              uploadDate={new Date(course?.created_at).toLocaleDateString('en-GB')}
            />
            
            <CourseStats 
              rating={courseData.rating}
              students={courseData.students}
              hours={course?.analytics?.total_video_duration && formatTimeHMS(course?.analytics?.total_video_duration, 2)}
            />
            
            <CourseContent 
              sections={course?.analytics?.section_count}
              lectures={course?.analytics?.video_count}
              duration={course?.analytics?.total_video_duration && formatTimeHMS(course?.analytics?.total_video_duration, 2)}
              assessmentCount={course?.analytics?.assessment_count}
            ...
            <CoursePanel title="What you'll learn">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
                {course?.objectives.map((item, index) => (
                  <CheckItem key={index} text={item.objective} />
                ))}
              </div>
            </CoursePanel>
            ...
            <CoursePanel title="Requirements">
              <ul className="list-disc pl-5 space-y-2">
                {course?.requirements.map((item, index) => (
                  <li key={index} className="text-gray-300">{item.requirement}</li>
                ))}
              </ul>
            </CoursePanel>
            
            {course?.subscription && (<CoursePanel title="This course includes:">
              ...
              <div className="mt-6 border-t border-navy-light pt-4 text-sm">
                <p className="text-red-400 font-medium">Note: Course Duration is {course.safe_period} days</p>
                <p className="text-gray-400 mt-1">If you have any complaints and need refund, you have to report it before the course duration ends which will happen only if you complete at least 80% of the lessons.</p>
              </div>
            </CoursePanel>)}
            
            <CoursePanel>
              <InstructorCard instructor={course?.instructor_details} />
            </CoursePanel>
            ...
            <CoursePanel>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  ...
                  <span className="font-bold mr-1">{courseData.rating}</span>
                  <span className="text-gray-400">course rating</span>
                  <span className="mx-2 text-gray-500">•</span>
                  <span className="text-gray-400">7K ratings</span>
                </div>
            ...
            <div className="mt-8">
              <h2 className="text-xl font-semibold mb-4">Similar Courses</h2>
