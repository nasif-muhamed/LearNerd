...
import { Calendar, Clock, Check, X, ArrowRight, User, MessageCircle, Video, Loader } from 'lucide-react';
...
const MyClassRoom = () => {
    ...
    const handleChatClick = async (studentId) => {
        try {
            setChatButtonLoading(true)
            const response = await api.get(`chats/one-one-room/${studentId}/`)
    ...
    const isRoomOpenToJoin = (start, end) => {
        let scheduledTime = new Date(start);
        scheduledTime.setMinutes(scheduledTime.getMinutes() - 5)
        let endingTime = new Date(end)
        endingTime.setMinutes(endingTime.getMinutes() + 5)
        const currentTime = new Date();
        return scheduledTime <= currentTime && currentTime <= endingTime;
    };

    const joinRoomText = (start, end) => {
        let scheduledTime = new Date(start);
        scheduledTime.setMinutes(scheduledTime.getMinutes() - 5)
        let endingTime = new Date(end)
        endingTime.setMinutes(endingTime.getMinutes() + 5)
        const currentTime = new Date();
        console.log('time:', scheduledTime, currentTime, endingTime)
        let text = ''
                    ...
                    sessions.map((session) => (
                        ...
                        <div className="flex-1">
                            ...
                            <div className="pl-12">
                                <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
                                    <span className={`px-2 py-0.5 rounded-full flex items-center text-xs ${getStatusBadge(session.status)}`}>
                                    ...
                                    {session.status !== 'completed' && (
                                        <div>
                                            {chatButtonLoading ? (
                                                <button className="btn-primary py-0.5 w-full text-center flex gap-2 items-center justify-center">
                                                    <Loader className="animate-spin"/> Chat
                                                </button>
                                            ) : (
                        
                                                <button onClick={() => handleChatClick(session.student?.id)} className="flex items-center gap-2 btn-primary py-0.5 w-full text-center">
                        ...
                        <div className="flex items-center justify-end space-x-2">
                            ...
                            {session.status === 'approved' && !session.is_upcoming && (                    
                                // <a 
                                //     href={`/video-call?session_id=${session.id}`}
                                //     target="_blank"
                                ...
                                // </a>
                                <a 
                                    href={isRoomOpenToJoin(session.scheduled_time, session.ending_time) ? `/video-call?session_id=${session.id}` : '#'}
                                    target={isRoomOpenToJoin(session.scheduled_time, session.ending_time) ?  `_blank` : ''}
                                    rel="noopener noreferrer"
                                    className={`flex items-center text-xs ${isRoomOpenToJoin(session.scheduled_time, session.ending_time) ? 'btn-primary' : 'btn-secondary cursor-not-allowed'}`}
                                >
                                    {console.log('isRoomOpenToJoin:', isRoomOpenToJoin(session.scheduled_time, session.ending_time))}
                                    <Video size={14} className="mr-1" />
                                    {joinRoomText(session.scheduled_time, session.ending_time)}
                                </a>
                            )}
